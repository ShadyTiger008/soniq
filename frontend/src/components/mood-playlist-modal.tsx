...
export function MoodPlaylistModal({ isOpen, onClose, onGenerate }: MoodPlaylistModalProps) {
  ...
  const [experienceType, setExperienceType] = useState<"single" | "small" | "medium" | "large">("medium");
  ...
  return (
    <Tooltip.Provider delayDuration={200}>
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent showCloseButton={false} className="!w-[60vw] !max-w-[70vw] p-0 overflow-hidden bg-black/40 backdrop-blur-[100px] border border-white/[0.08] rounded-[2.5rem] h-[80vh] flex flex-col shadow-[0_0_120px_rgba(0,0,0,0.8)]">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="space-y-0.5">
                <div className="flex items-center gap-2">
                  <DialogTitle className="text-lg font-black text-white tracking-tighter uppercase italic font-space-grotesk">Soniq AI Buddy</DialogTitle>
                  <Tooltip.Root>
                    ...
                    <Tooltip.Portal>
                      ...
                      >
                        <p className="text-xs text-white/80 leading-relaxed font-bold tracking-tight">
                          Your personal music concierge. We use Soniq Buddy AI to analyze your mood, activity, and preferences to curate a perfectly synced queue.
                        </p>
                ...
                <DialogDescription className="text-[10px] font-bold tracking-[0.2em] text-white/20 uppercase">Next-Gen Curation Engine</DialogDescription>
              </div>
            </div>
            <DialogClose asChild>
          ...
          <ScrollArea className="flex-1 min-h-0">
            <div className="space-y-8 p-6">
              ...
              <section>
                <h3 className="flex items-center gap-2 text-[10px] font-bold tracking-[0.2em] text-white/30 uppercase mb-4">
                  <MessageSquare className="h-3 w-3" /> Describe Vibe
                </h3>
                <textarea 
                  value={vibeText}
          ...
          <div className="shrink-0 p-6 bg-black/40 border-t border-white/[0.05] backdrop-blur-3xl space-y-4 relative z-30">
            ...
            <div className="flex items-center justify-center gap-3">
               ...
               <p className="text-[8px] font-black text-white/10 tracking-[0.5em] uppercase whitespace-nowrap">
                  Elite AI Curation Engine
               </p>
               <div className="h-px flex-1 bg-gradient-to-r from-transparent via-white/5 to-transparent" />
            </div>
          </div>
...
function ToggleOption({ label, enabled, onChange, tip }: { label: string, enabled: boolean, onChange: (v: boolean) => void, tip: string }) {
  return (
    <Tooltip.Root>
      <Tooltip.Trigger asChild>
        <div className="flex items-center justify-between gap-3 bg-white/[0.02] border border-white/[0.05] rounded-2xl px-5 py-4 hover:bg-white/[0.05] transition-all cursor-help group/toggle">
          <span className="text-[10px] font-black tracking-widest text-white/30 uppercase group-hover/toggle:text-white/60 transition-colors">{label}</span>
          <Switch.Root 
            checked={enabled} 
            onCheckedChange={onChange}
            className={`w-10 h-5 rounded-full relative transition-all outline-none cursor-pointer ${enabled ? "bg-primary shadow-[0_0_15px_rgba(29,185,84,0.3)]" : "bg-white/10"}`}
          >
      ...
      <Tooltip.Portal>
        <Tooltip.Content 
          className="z-[200] max-w-[200px] bg-black/90 backdrop-blur-3xl border border-white/10 p-4 rounded-2xl shadow-2xl animate-in fade-in zoom-in-95 duration-200"
          sideOffset={10}
        >
          <p className="text-[10px] text-white/80 font-bold leading-relaxed tracking-wider uppercase italic">{tip}</p>
          <Tooltip.Arrow className="fill-black/90" />
        </Tooltip.Content>
      </Tooltip.Portal>
    </Tooltip.Root>
